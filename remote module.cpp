...
bool saveIRSignalNamed(const String &hex, const String &protocol,
    ...
    if (SD.exists(fpath)) {
        int idx = 1;
        while (SD.exists(
            "/remote module/" + safeName + "_" + String(idx) + ".ir"))
            idx++;
...
void drawStaticFrames() {
    M5Cardputer.Display.drawRect(
        2, SIGNAL_BOX_Y, SCREEN_W - 4, SIGNAL_BOX_H, COLOR_GRAY);
    M5Cardputer.Display.drawRect(
        2, LIST_BOX_Y, SCREEN_W - 4, LIST_BOX_H, COLOR_GRAY);
}

void redrawSignalContent() {
    int y = SIGNAL_BOX_Y;
    M5Cardputer.Display.fillRect(
        3, y + 1, SCREEN_W - 6, SIGNAL_BOX_H - 2, COLOR_BG);
    ...
    if (lastHex.length() > 0) {
        M5Cardputer.Display.setTextColor(
            newSignalReceived ? COLOR_GREEN : COLOR_WHITE);
...
void redrawListContent() {
    // Başlık alanı
    M5Cardputer.Display.fillRect(
        3, LIST_BOX_Y + 1, SCREEN_W - 6, 12, COLOR_BAR);
    M5Cardputer.Display.drawFastHLine(
        3, LIST_BOX_Y + 13, SCREEN_W - 6, COLOR_GRAY);
    ...
    M5Cardputer.Display.fillRect(
        3, LIST_START_Y - 1,
        SCREEN_W - 10, LIST_BOX_H - 15, COLOR_BG);

    // Dosya listesi
    for (int i = 0;
         i < LIST_VISIBLE && (i + scrollOffset) < fileCount;
         i++) {
        ...
        if (sel) {
            M5Cardputer.Display.fillRoundRect(
                4, y - 1, SCREEN_W - 10, LIST_ROW_H, 2, COLOR_DARK);
    ...
    if (fileCount > LIST_VISIBLE) {
        ...
        int segY = LIST_START_Y +
                   (selectedFile * (barH - segH)) /
                   max(1, fileCount - 1);
        M5Cardputer.Display.drawFastVLine(
            SCREEN_W - 4, LIST_START_Y, barH, COLOR_GRAY);
        M5Cardputer.Display.drawFastVLine(
            SCREEN_W - 4, segY, segH, COLOR_WHITE);
    }
}
...
void drawSendFlash() {
    M5Cardputer.Display.fillRect(
        SCREEN_W - 80, SIGNAL_BOX_Y + 24, 76, 14, COLOR_GRAY);
...
void drawSaveFlash(bool ok) {
    M5Cardputer.Display.fillRect(
        SCREEN_W - 72, SIGNAL_BOX_Y + 24, 68, 14,
...
void updateNamingInputBox() {
    int boxY = TOP_BAR_H + 28;
    M5Cardputer.Display.fillRect(
        5, boxY + 1, SCREEN_W - 10, 20, COLOR_BG);
    M5Cardputer.Display.setTextColor(COLOR_GREEN);
    M5Cardputer.Display.setTextSize(1);
...
void handleNamingMode() {
    ...
    if (enterPressed(ks)) {
        namingMode      = false;
        bool ok         = saveIRSignalNamed(
            lastHex, lastProtocol, lastValue, pendingSaveName);
        pendingSaveName = "";
...
void handleIRReceive() {
    if (irrecv.decode(&results)) {
        if (results.value != 0xFFFFFFFFFFFFFFFF &&
            ...
            lastProtocol      = typeToString(
                results.decode_type, results.repeat);
